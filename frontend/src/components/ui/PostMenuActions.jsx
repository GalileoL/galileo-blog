...
const PostMenuActions = ({ post }) => {
  ...
  const featureMutation = useMutation({
    ...
    onMutate: async () => {
      ...
      const prev = queryClient.getQueryData(["post", post.slug]);

      // Optimistically update the post's isFeatured status
  ...
  const handleFeaturePost = () => {
    // check if the user is authenticated
    if (!user) {
      toast.error("You need to be logged in to feature posts");
      navigate("/login");
      return;
    }
    ...
    <div className="flex flex-col gap-2">
      ...
      {isPending ? (
        ...
        <button
          ...
          className="flex items-center gap-2 py-2 text-sm cursor-pointer disabled:cursor-not-allowed"
